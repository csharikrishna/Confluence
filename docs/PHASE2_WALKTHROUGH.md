...
**Update (2026-09-04) — archived, not deleted.** Once Mongo (below) was verified as a working, fully-managed, genuinely-free alternative, keeping two dormant durable backends on `main` stopped being future-proofing and started being clutter — Mongo covers everything CouchDB would here, with no server to self-host. `couchdb_storage.py`, `docker-compose.couchdb.yml`, and its test suite were moved to the `archive/couchdb-backend` git branch (`git checkout archive/couchdb-backend` to retrieve them in full); `db_backend.py` no longer imports the module, and `STORAGE_BACKEND=couchdb` now safely falls back to SQLite instead of activating it. Everything above this note is left exactly as written at the time — it's still an accurate record of the design and the real bug it caught.
...
**167 offline tests passing** (1 skipped — an upload-path test that only runs if `google-api-python-client` happens to be installed locally; count reflects the CouchDB backend's later archival, see update above). All new modules verified live: server starts cleanly, `/health` correctly reports `sqlite` as the active backend and both new integrations as "not configured" until you add credentials, `/environment` still works end-to-end against real upstream data — and at the time, both the CouchDB and MongoDB backends were verified against real running instances, not just mocks.

---

## Part 5 — Repo cleanup, and catching a real production outage

A follow-up pass triggered by two separate asks: keep `main` free of dormant code that isn't earning its place, and do a full pass over the live deployment to make sure nothing was silently broken.

### CouchDB archived, not deleted
`couchdb_storage.py`, `docker-compose.couchdb.yml`, and their 24-test suite were removed from `main` and now live on the **`archive/couchdb-backend`** branch (`git checkout archive/couchdb-backend` retrieves the complete, working implementation). The reasoning: Mongo already covers everything a durable backend needs here — managed, free, no server to self-host — so keeping two dormant durable backends on `main` was clutter, not future-proofing. `db_backend.py` no longer imports the module; a leftover `STORAGE_BACKEND=couchdb` (e.g. a stale env var) now safely falls back to SQLite instead of erroring at import time. See the dated update in Part 4 above and the README's "Archived backends" section for the full reasoning.

### Production was silently down — here's how it was found
Routine post-push verification (hitting the live `/health` endpoint directly, not just trusting a green build) turned up `"status":"degraded"`, `"history_store":"mongodb@unknown-host"`, `"history_store_status":"unreachable"`. Two distinct, real bugs were behind it:

1. **`pymongo` was never actually installed on Render.** Confirmed by calling `/environment/history` directly and reading the raw error: `"pymongo isn't installed. Run: pip install -r requirements-mongo.txt"` — that exact string only comes from `mongo_storage.py`'s import guard genuinely failing. `render.yaml` has always declared `buildCommand: pip install -r requirements.txt -r requirements-mongo.txt`, but Render only auto-adopts a repo's `render.yaml` build command when a service is first created *from* a Blueprint. This service predates the Mongo work, so its dashboard-configured Build Command had silently stayed on the old `pip install -r requirements.txt` the whole time — `render.yaml`'s declaration was correct in the repo and irrelevant in practice.
2. **`MONGODB_URI` was separately malformed** — non-empty but not parsing to a hostname (`_safe_host_desc()`'s `urlparse(...).hostname or "unknown-host"` fallback only fires on a genuinely unparseable, non-empty value; an unset URI returns a different message entirely). Likely a wrapped quote, stray whitespace, or a duplicated `MONGODB_URI=` prefix carried over from a dashboard paste.

Fix: correct the Build Command in Render's **Settings** tab (not Environment — a different tab, easy to miss), re-paste `MONGODB_URI` fresh rather than editing in place, then **Clear build cache & deploy** rather than a plain redeploy (to avoid reusing a cached venv still missing pymongo). Confirmed fixed live: `/health` now reports `"status":"healthy"`, `"history_store_status":"connected"`.

### What this meant for Phase 2A's history data
With `pymongo` unavailable, every `storage.save_snapshot()` call had been failing since whenever the Build Command drifted — caught and logged as a warning by `_persist_and_log`'s best-effort try/except (by design: a storage failure must never break the `/environment` response), which is exactly why nothing surfaced until someone went looking at `/health` and `/environment/history` directly. Querying Chennai Coast's history turned up only **36 rows total** across the full Jan–Sep range: 31 sharing one identical `generated_at` timestamp (`2026-09-03T10:00:00Z`) and 5 with genuine, irregularly-spaced timestamps from the day the fix landed. Read `app.py`'s route handler to check the obvious theory (repeated cache-hit requests each writing a duplicate row) — ruled out directly by the code: persistence is only scheduled `if data and not meta.get("cache_hit")`, and the ingestion cron always passes `bypass_cache=true`. 31 independently-fetched snapshots landing on the exact same second isn't something the normal request path can produce; the far more likely explanation is one snapshot fetched once and saved in a direct, scripted loop (consistent with the earlier Mongo write/read verification testing), not the hourly cron or organic traffic. Net effect either way: **there is no real historical trend data before the fix landed** — Phase 2A's history effectively starts from today. The 31-row batch is still sitting in production, left alone pending an explicit decision to purge it; whether `RENDER_APP_URL` is actually set as a repo secret (required for `ingest_history.yml` to do anything at all — it exits `0` silently if missing) is also still unconfirmed.

### Full commit-history security audit
Separately requested: confirm nothing sensitive ever leaked into git, not just the current working tree. Checked all commits across every branch (`main`, `archive/couchdb-backend`, all fetched `dependabot/*` branches) — no `.env`-shaped file was ever committed at any point in history; pickaxe-searched (`git log --all -S"<value>"`) for the specific credentials that had appeared in plaintext chat during this project's Mongo setup — zero hits, meaning they were never staged, let alone committed; swept every historical diff for credential-embedded connection strings, non-placeholder API keys, webhook URLs, and private-key blocks, and swept every historical filename for credential-shaped names (`*.pem`, `*.key`, `id_rsa`, `credentials.json`) — all clean except the intentional `.env.example` template. Nothing required removal, so no history rewrite (`git filter-repo`/BFG plus a force-push) was needed.
...
After the Part 4 science/storage pass: **still 9/10, but a more defensible 9.** The alert science now cites real published standards instead of invented bands, the storage architecture is genuinely pluggable (Mongo confirmed as the durable option; CouchDB proved out the pattern and was then archived — see the update above), and Google Drive backup is real, tested (against mocks), optional code — not a promise. It stays short of 10 for exactly the reasons in Part 3: nothing here can install/run CouchDB, create Google credentials, attach a paid disk, get a domain expert's sign-off, or watch uptime with a dedicated third-party service. Those remain real-world actions, not code gaps.

After Part 5: **the code didn't change grade, but the process did.** The Mongo outage was invisible to every check that had been run up to that point — tests passing, a build that "succeeds," even the earlier local write/read verification against Atlas — because none of those exercise Render's actual configured Build Command or read the live `/health` JSON back. The gap wasn't a code defect; it was that nobody had looked at production directly since the initial Mongo wiring. That's twice now on this project (the earlier `-e` in `requirements.txt` was the first) that "verified locally" and "confirmed in production" turned out to be different claims. Worth taking as a standing rule going forward, not just a one-off fix: after any deploy-affecting change, read the live `/health` response itself — a green build and a healthy service aren't the same thing, and the difference is only visible from outside the code.
